...
`)):null})();if(result){logErrors("\u26D4\uFE0F Sorry!",result),nodeLogger.logger.line(1);return}if(info2.frameworkPackageName==="@storybook/nextjs"){printInfo("\u{1F37F} Just so you know...",tsDedent.dedent`
      ...
      `);try{let storybookVersion=await packageManager.getInstalledVersion("storybook");uniqueDependencies.push(`@storybook/nextjs-vite@^${storybookVersion}`);}catch{console.error("Failed to install @storybook/nextjs-vite. Please install it manually");}}let v8Version=await packageManager.getInstalledVersion("@vitest/coverage-v8"),istanbulVersion=await packageManager.getInstalledVersion("@vitest/coverage-istanbul");!v8Version&&!istanbulVersion&&(printInfo("\u{1F648} Let me cover this for you",tsDedent.dedent`
      ...
      `),uniqueDependencies.push("@vitest/coverage-v8"));let versionedDependencies=uniqueDependencies.map(p=>p.includes("vitest")&&vitestVersionSpecifier?`${p}@${vitestVersionSpecifier}`:p);if(versionedDependencies.length>0&&(await packageManager.addDependencies({type:"devDependencies",skipInstall:!0},versionedDependencies),nodeLogger.logger.line(1),nodeLogger.logger.plain(`${step} Installing dependencies:`),nodeLogger.logger.plain("  "+versionedDependencies.join(", "))),await packageManager.installDependencies(),nodeLogger.logger.line(1),options.skipInstall)nodeLogger.logger.plain("Skipping Playwright installation, please run this command manually:"),nodeLogger.logger.plain("  npx playwright install chromium --with-deps");else {nodeLogger.logger.plain(`${step} Configuring Playwright with Chromium (this might take some time):`),nodeLogger.logger.plain("  npx playwright install chromium --with-deps");try{await packageManager.executeCommand({command:"npx",args:["playwright","install","chromium","--with-deps"]});}catch{console.error("Failed to install Playwright. Please install it manually");}}let fileExtension=allDeps.typescript||await findFile("tsconfig",[...EXTENSIONS,".json"])?"ts":"js",vitestSetupFile=resolve(options.configDir,`vitest.setup.${fileExtension}`);if(fs.existsSync(vitestSetupFile)){logErrors("\u{1F6A8} Oh no!",tsDedent.dedent`
...
`)}
    ...
    `);let vitestWorkspaceFile=await findFile("vitest.workspace",[".ts",".js",".json"])||await findFile("vitest.projects",[".ts",".js",".json"]),viteConfigFile=await findFile("vite.config"),vitestConfigFile=await findFile("vitest.config"),vitestShimFile=await findFile("vitest.shims.d"),rootConfig=vitestConfigFile||viteConfigFile;fileExtension==="ts"&&!vitestShimFile&&await fs4.writeFile("vitest.shims.d.ts",isVitest4OrNewer?'/// <reference types="@vitest/browser-playwright" />':'/// <reference types="@vitest/browser/providers/playwright" />');let getTemplateName=()=>isVitest4OrNewer?"vitest.config.4.template.ts":isVitest3_2To4?"vitest.config.3.2.template.ts":"vitest.config.template.ts";if(vitestWorkspaceFile){let workspaceTemplate=await loadTemplate("vitest.workspace.template.ts",{EXTENDS_WORKSPACE:viteConfigFile?relative(dirname(vitestWorkspaceFile),viteConfigFile):"",CONFIG_DIR:options.configDir,SETUP_FILE:relative(dirname(vitestWorkspaceFile),vitestSetupFile)}).then(t=>t.replace(`
...
`+formattedContent);}else logErrors("\u{1F6A8} Oh no!",tsDedent.dedent`
      ...
      `);}else {let newConfigFile=resolve(`vitest.config.${fileExtension}`),configTemplate=await loadTemplate(getTemplateName(),{CONFIG_DIR:options.configDir,SETUP_FILE:relative(dirname(newConfigFile),vitestSetupFile)});nodeLogger.logger.line(1),nodeLogger.logger.plain(`${step} Creating a Vitest config file:`),nodeLogger.logger.plain(`  ${newConfigFile}`);let formattedContent=await common.formatFileContent(newConfigFile,configTemplate);await fs4.writeFile(newConfigFile,formattedContent);}if(info2.addons.find(addon=>addon.includes(addonA11yName)))try{nodeLogger.logger.plain(`${step} Setting up ${addonA11yName} for @storybook/addon-vitest:`),await execa("storybook",["automigrate","addon-a11y-addon-test","--loglevel","silent","--yes","--skip-doctor",...options.packageManager?["--package-manager",options.packageManager]:[],...options.skipInstall?["--skip-install"]:[],...options.configDir!==".storybook"?["--config-dir",`"${options.configDir}"`]:[]],{stdio:"inherit"});}catch(e){logErrors("\u{1F6A8} Oh no!",tsDedent.dedent`
